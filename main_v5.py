...
# Readline code obtained from https://github.com/pyserial/pyserial/issues/216#issuecomment-369414522
# Intended to deal with slowness of pyserial's readline command.
class ReadLine:
    def __init__(self, s):
        self.buf = bytearray()
        self.s = s
...
class Bridge():
    ...
    def save_to_file(self, sensor, data, lifo):
        """
        ...
        try:
            ...
            with open(file_path, 'a') as data_file:
                ...
                lifo[sensor['name']].append(data.values())
        except:
            msg_with_time = self.create_log(traceback.format_exc(), sensor['name'])
        ...
        while True:
            ...
            if len(row) > 0:
                if row[0] in self.udp_sensors.keys():
                    ...
                    # print (data)
                    self.manage_data(data, sensor, lifo, show_no_internet_error)
            #time.sleep(1)
    ...
    def manage_data(self, data, sensor, lifo, show_no_internet_error):
        """
        ...
        """
        self.save_to_file(sensor, data, lifo)

    def send_data_files_wrapper(self, sensor, temp_file, lifo):
        while True:
            try:
                self.send_data_files(sensor, temp_file, lifo)
            except Exception as e:
                print ("Problem sending files.")
                
            time.sleep(int(self.server_options['interval']))

    def send_data_files(self, sensor, temp_file, lifo):
        """
        Send temporary data to the server.
        :return:
        """
        
        # Need to uncomment this for server send (8/23/24)
        ...
        #SESSION = True
        #print ("1")
        #listlifo = list(lifo[sensor['name']])
        #listlifo.pop(0)
        #listlifo = lifo[sensor]
        #print ("2")
        #print (sensor)
        #print ("Test lifo", listlifo)
        header = sensor['header'].split(',')
        
        #print (header)
        header.append('datetime')
        msg = 'Sending sensor {} data'.format(sensor['name'])
        msg_with_time = self.create_log(msg, sensor['name'])
        # print(msg_with_time)
        
        # Temporary code to load up our stack. (2024/08/23)
        """
        ...
        print ("Blah.", listlifo)
        """
        while len(lifo[sensor['name']]) != 0:
        #while len(listlifo) != 0:
            #pdb.set_trace()
            #print (len(listlifo))
            #lifodata = listlifo.pop()
            lifodata = lifo[sensor['name']].pop()
            ...
            #data = dict(zip(header, lifodata.strip().split(',')))
            data = dict(zip(header, lifodata))
            #print (data)
            #msg4 = str(data)
            ...
            try:
                ...
                #msg5 = "Send succeeded."
                #msg_with_time = self.create_log(msg5, sensor['name'])
            except:
                msg5 = "Sending data failed!"
                msg_with_time = self.create_log(msg5, sensor['name'])
                response = False
            if response == False:
                lifo.append(lifodata)
                #print (lifo)
                return
            else:
                if len(lifo[sensor['name']])%50 == 0:
                
                    #print (str(lastlinenum) + "/" + str(len(lines)))
                    msg3 = "Left in buffer: " + str(len(lifo[sensor['name']]))
                    msg_with_time = self.create_log(msg3, sensor['name'])
...
if __name__ == "__main__":
    ...
    def run_process(app):
        ...
        for sensor in config['sensors_config']:
            ...
            # Create a manager list, used effectively as a stack using only pop and append.
            lifo = []
            lifo = manager.list()
            #lifo.append(sensor['name'])
            #print (sensor['name'], lifo)
            lifodict[sensor['name']] = lifo
            ...
            if sensor['type'] == 'COM port':  # To exclude UDP reading
                ...
                p = Process(target=bridge.read_com, args=(sensor, lifodict,))
                p.name = sensor['label']
                p.daemon = True
                processes.append(p)
                
            temp_file = glob.glob(config['basic_options']['data_path'] + '/*' + sensor['name'] + '.csv')[0]
                
            if config["server_options"]["send_data"]:
                p1 = Process(target=bridge.send_data_files_wrapper, args=(sensor,temp_file,lifodict,))
                p1.name = 'Server Send'
                p1.daemon = True
                processes.append(p1)
                    

        p0 = Process(target=bridge.read_udp, args=(lifodict,))
        p0.name = 'GPS Position'
        p0.daemon = True
        processes.append(p0)
        
        temp_file = glob.glob(config['basic_options']['data_path'] + '/*gpsposition.csv')[0]
        ...
        if config["server_options"]["send_data"]:
            ...
            temp_files = glob.glob(config['basic_options']['data_path'] + '/*.csv')
            print (temp_files)
            for temp_file in temp_files:  # get com port from temp file

                if not path.exists(temp_file):
                    continue
                sensors = [l for l in config['sensors_config']
                           if l['name'] in temp_file]
                ...
                #print(temp_file)
                #print(sensor)
                p1 = Process(target=bridge.send_data_files_wrapper, args=(sensor,temp_file,))
                p1.name = 'Server Send'
                p1.daemon = True
                processes.append(p1)
        """    
